...
import { RequestSearchProduct, ResponseSearchProduct, ResponseSearchProductDetail } from "./typeApi/SearchProductType";
...
export const getSearchProductDetail = async (id: string): Promise<ResponseSearchProductDetail> => {
  const response = await api.get(`/crawling/${id}`);
  return response.data
}

export const getExchangeCurrency = async (): Promise<ResponseExchangeCurrency> => {
  const response = await api.get('/exchange_new');
  return response.data
}

export const getWeightPerPrice = async (weight: number): Promise<ResponseWeightPerPrice> => {
  const response = await api.get(`/price/weight?weight=${weight}`);
  return response.data
}

export const calculatePrice = async (data: RequestCalculatePrice): Promise<ResponseCalculatePrice> => {
